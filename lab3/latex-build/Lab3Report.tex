...
The bouncing-ball experiment is the canonical hybrid model (continuous flight with gravity, discrete bounce resets).The lab model instantiates the template from Canvas for reachability experiments below.
...
\medskip
\noindent \textbf{Summary:} With a coarse partition (\texttt{pc}=0.5), the target is covered by the reachable set, so exclusion cannot be claimed. With the finer partition (\texttt{pc}=0.05), the target point is outside the reachable set, and we can visually conclude it is unreachable at this precision.
...
\afterprompt
Command form for reachability check:
\begin{lstlisting}[style=phaver]
target = bball.{falling & x==55/100 & v==0};
...
\medskip
\noindent The sweep shows the expected precision-runtime tradeoff: as \texttt{pc} decreases, the number of symbolic locations and CPU time increase sharply, but the reachable enclosure becomes tighter. In our runs, reachability of state comes to a conclusion between \texttt{pc}=0.1 (reachable) and \texttt{pc}=0.05 (not reachable), which is consistent with refinement removing spurious reachability.
...
% \textit{We have to encode the heating model in PHAVer syntax. Add a clock \texttt{t} (init 0, derivative 1 in both locations, value retained across jumps). Enforce $0\leq t\leq 10$ in both locations. Plot reachable states: $x$-axis time, $y$-axis temperature.}
\afterprompt
...
\afterprompt
\noindent This experiment models a heating system starting in the \emph{cool} location with clock $t=0$ and temperature $x=22$. We are to check: is it ever possible for $x<20$ within $t=0.98$ seconds? We encode the "bad" set as the slice $t=0.98$ and $x<20$, and use \texttt{is\_reachable} to see if this state is attainable for several values of the partition constraint (\texttt{pc}).
...
Sweeping across \texttt{pc}, our results show that for \texttt{pc}=1.0 and \texttt{pc}=1.2, the bad set remains \texttt{not reachable}, but when \texttt{pc}=1.2 or larger, the over-approximation becomes too coarse and the bad set is reported as \texttt{reachable}. Thus, the largest value of \texttt{pc} (within $0.1$ of the true threshold) for which the bad set is not reachable is \texttt{pc}=1.2.

\noindent \textbf{Result:} The largest partition constraint for which the system reliably proves the property (unreachability of the bad set) is \texttt{pc}=1.2; for \texttt{pc}$\geq$1.2, the abstraction is too coarse to exclude $x<20$ at $t=0.98$.
...
\subsection*{Quickly checking using equations of motion}
Uniform acceleration segments satisfy the usual SUVAT relations; they are useful to sanity-check extreme speeds and stopping distances against the reachable set, not as a substitute for PHAVerLite proofs. The equations of motion are:
\begin{align}
...
\begin{lstlisting}[style=phaver,caption={C.3: PHAVerLite output summary for the three property checks with pc=3.0},label={fig:c3_output}]
  p1_bad is reachable.
  p2_bad is reachable.
  p3_bad is reachable.
  \end{lstlisting}
...
\noindent Here \texttt{v >= 0.001} in P1 separates ``still moving'' from a full stop at $v=0$. At \texttt{pc=3.0}, PHAVerLite reported \textbf{reachable} for all three bad sets; therefore \textbf{none} of the three properties is formally proven as stated (unreachability would be required for a sound proof). Reachability here may be due to over-approximation, but it does not establish safety. A tighter \texttt{pc} would be needed to refine our conclusion.
...
\begin{lstlisting}[style=phaver,caption={C.3: PHAVerLite output summary for the three property checks with pc=1.2},label={fig:c3_output_12}]
  p1_bad is not reachable.
  p2_bad is not reachable.
  p3_bad is reachable.
  \end{lstlisting}

\subsection*{Verifying Properties from Physics}

\subsubsection*{P1: Stops before the second intersection}
We verify Property~1 using physics. Let $x_b\in[250,300]$ be the braking-trigger position. From rest in the acceleration phase,
\[
v_b^2 = 2a_{\mathrm{acc}}x_b = 2(3.5)x_b.
\]
In the braking phase with $a_{\mathrm{br}}=-5.5$, the stopping distance is
\[
d_{\mathrm{stop}}=\frac{v_b^2}{2\cdot 5.5}.
\]
So the stop position is
\[
x_{\mathrm{stop}} = x_b + d_{\mathrm{stop}}
= x_b + \frac{2\cdot 3.5\,x_b}{2\cdot 5.5}
= x_b\!\left(1+\frac{7}{11}\right)
= \frac{18}{11}x_b.
\]
Worst case is $x_b=300$, giving
\[
x_{\mathrm{stop,max}}=\frac{18}{11}\cdot 300\approx \SI{490.9}{\meter}<\SI{500}{\meter}.
\]
Thus, the concrete dynamics satisfy Property~1. This agrees with the refined PHAVer result (\texttt{pc}=1.2: bad set not reachable) and explains why the coarse \texttt{pc}=3.0 run can be conservative.

\subsubsection*{P2: $v \leq \SI{46.5}{\meter\per\second}$ during braking}
We verify Property~2 using physics. The car starts from rest and accelerates at \SI{3.5}{\meter\per\second\squared} for at most \SI{300}{\meter}, so the largest possible entry speed into braking is
\[
v_{\max}=\sqrt{2as}=\sqrt{2\cdot 3.5\cdot 300}\approx\SI{45.8}{\meter\per\second}<\SI{46.5}{\meter\per\second}.
\]
Since braking only decreases speed, Property~2 is satisfied by the concrete dynamics. This is consistent with the refined PHAVer run (\texttt{pc}=1.2: bad set not reachable), while the coarse \texttt{pc}=3.0 verdict is conservatively inconclusive.

\subsubsection*{P3: $v \leq \SI{40.0}{\meter\per\second}$ during braking}
We verify Property~3 using physics. Braking can begin anywhere in $x_b\in[250,300]$, so the braking-entry speed range is
\[
v_b=\sqrt{2\cdot 3.5\,x_b}\in
\left[\sqrt{2\cdot 3.5\cdot 250},\sqrt{2\cdot 3.5\cdot 300}\right]
\approx [\SI{41.8}{\meter\per\second},\SI{45.8}{\meter\per\second}].
\]
Because the minimum possible entry speed already exceeds \SI{40}{\meter\per\second}, the violation condition for Property~3 is physically possible (indeed immediately at braking entry). So Property~3 is not valid for the concrete model, matching PHAVer reachability at both \texttt{pc}=3.0 and \texttt{pc}=1.2.
...
\subsection*{C.4: 0--60\,MPH car speed estimate}
...
\section*{Conclusion}
Across Parts A and B we saw the central practical lesson of PHAVerLite reachability: decreasing \texttt{pc} substantially increases cost (locations and runtime) but can sharpen the reachable enclosure enough to change the reachability verdict for a boundary target. For the heating model, sharpening provided a concrete way to locate where the abstraction stops being strong enough to exclude a time-slice counterexample.

\noindent In Part C, we encoded environmental uncertainty as nondeterminism in the braking trigger and checked properties as bad-state reachability queries. In our runs, the three C.3 bad sets were reachable at \texttt{pc}=3.0 (hence not proven, or not proved "unprovable"). For C.4 bracketing approach produced a narrow and interpretable 0--60\,mph time interval, so we just which could tell us some interesting facts about how this the roadside "STOP" indicator placement could be optimized considering the various car models we created for based on performance too!
...
\textbf{Files submitted:}
\begin{itemize}
  \item \texttt{SHIH\_A3.pha}
  \item \texttt{SHIH\_B2.pha}
  \item \texttt{jkhare\_C3.pha}
  \item \texttt{jkhare\_C4.pha}
\end{itemize}
