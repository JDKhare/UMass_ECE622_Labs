...
//
// Usage:
//   ./lab1_parser <verilog.v> <target_bits> <unroll_k>
//
// Produces:
//   out.dimacs  (CNF reachability query)
//   out.nodes   (varID:nodeName,timeframe)
//
// Notes:
// - Designed for structural netlists described as And-Inverter-Graph (AIG) and
...
// - Initial state is left UNCONSTRAINED in this sample (easy to add constraints if needed).
...
int main(int argc, char** argv) {
  if (argc != 4) {
    cerr << "Usage: " << argv[0] << " <verilog.v> <target_bits> <unroll_k>\n";
    return 1;
  }
  ...
  // Write out.nodes
  {
    ...
    if (!fout) { cerr << "ERROR: cannot write out.nodes\n"; return 1; }
  ...
  // Write out.dimacs
  {
    ofstream fout("out.dimacs");
<De
    if (!fout) { cerr << "ERROR: cannot write out.dimacs\n"; return 1; }
    std::ofstream fdim(dimacs_path);
    std::ofstream fnodes(nodes_path);
  ...
  cerr << "Wrote out.dimacs and out.nodes\n";
