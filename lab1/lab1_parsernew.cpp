...
// Run SAT solver (MiniSAT or PicoSAT) and capture timing + logs
{
  ...
  std::string solver = basename(minisat_path); // keep your arg name
  std::string log_path = out_dir + "/solver.log";  // choose your directory variable

  std::string cmd;
  if (solver.find("picosat") != std::string::npos) {
    // PicoSAT: solution to stdout, stats often to stderr (varies by flags)
    // Capture solution in sat_path, stats in log_path.
    // -v makes it print more info (including timing) as comment lines.
    cmd = minisat_path + " -v " + dimacs_path +
          " > " + sat_path + " 2> " + log_path;
  } else {
    // MiniSAT: prints progress/timing to stdout, writes model to sat_path
    // Capture stdout/stderr into log_path so you can compare.
    cmd = minisat_path + " " + dimacs_path + " " + sat_path +
          " > " + log_path + " 2>&1";
  }
  ...
  if (rc != 0) {
    std::cerr << "WARNING: SAT solver returned non-zero exit code " << rc << "\n";
  }

  // Save the measured wall time (ms) to a file for easy parsing/plotting
  std::ofstream tf(out_dir + "/wall_time_ms.txt");
  tf << wall_ms << "\n";

  std::cerr << "Solver wall time: " << wall_ms << " ms\n";
  std::cerr << "Solver output: " << sat_path << "\n";
  std::cerr << "Solver log:    " << log_path << "\n";
}
