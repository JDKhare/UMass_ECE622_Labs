...
You can also run this underlying standalone tool directly if you prefer writing your own logic constraints!

### Compiling `v2dimacs`

```bash
g++ -std=c++11 -O2 v2dimacs.cpp -o v2dimacs
```
...
```bash
# Usage Pattern:
./v2dimacs <verilog_file> <target_state_bitstring> <unroll_depth> <init_state_bitstring> <out_dimacs> <out_nodes>

# Example 1: Convert stoplight1 to DIMACS exactly at 15 transitions
./v2dimacs ./verilog_src/stoplight1.v 0100 15 0000 stoplight1_run.dimacs stoplight1_run.nodes

# Example 2: Convert a 2-bit circuit looking for state 11 at 10 transitions
./v2dimacs ./verilog_src/ex1.v 11 10 00 my_custom.dimacs my_custom.nodes
```

Then, the bash wrapper automatically passes the `.dimacs` file to `minisat` or `picosat`, timing the execution, reading the raw output, and verbosely printing the findings to the terminal. You get a clean `SAT | Target: 1010 | Depth: 5` interface while keeping all deep technical files preserved in your optional logging directory.
