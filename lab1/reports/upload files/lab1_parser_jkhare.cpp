...
//
// target_bits behavior:
//   - If target_bits is "-"  : DO NOT constrain target at time k (base CNF for Part C sweep).
//   - Else                   : constrain target at time k to the provided 0/1 bitstring.
//
// Produces:
...
int main(int argc, char** argv) {
  ...
  // NEW: allow "-" meaning "no target constraint"
  bool constrain_target = (target != "-");
  ...
  // target bits must match number of state regs (ONLY if we constrain target)
  if (constrain_target && (int)target.size() != (int)state_regs.size()) {
  ...
  // NEW: Target constraint at time k is OPTIONAL
  if (constrain_target) {
    for (int i = 0; i < (int)state_regs.size(); i++) {
      ...
      if (b == '1') cnf.add_clause({ v});
        ...
        cerr << "ERROR: target_bits must be only 0/1 (or '-' for no target)\n";
        return 1;
      }
    }
  }

  // Constrain initial state at time 0 (ALWAYS)
  for (int i = 0; i < (int)state_regs.size(); i++) {
    char b = init_bits[i];
  ...
  // Write out.dimacs
  {
    ...
    fout << "c File: " << vfile << "\n";
    fout << "c Unroll k = " << k << "\n";
    if (constrain_target) fout << "c Target at time k = " << target << "\n";
    else                 fout << "c Target at time k = (none; base CNF)\n";
  ...
  // Run MiniSat on generated CNF (still fine; base CNF will just be SAT usually)
  {
  ...
  cerr << "\n";
  if (!constrain_target) {
    cerr << "NOTE: target constraint DISABLED (target_bits='-').\n";
  }
  return 0;
}
