...
Run after: bash run_vgs_id_vds_sweep.sh LAB4 plots/<stamp>
"""
...
def main():
    ap = argparse.ArgumentParser(description="Id vs Vds family from vg_* sweep dirs.")
    ap.add_argument(
        "-d",
        "--plot-dir",
        type=Path,
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Output PNG (default: <plot-dir>/nmos_phas_id_vds_family.png)",
    )
    args = ap.parse_args()
    root = args.plot_dir
    if not root.is_dir():
        ...
        return 1
    out = args.output or (root / "nmos_phas_id_vds_family.png")
    ...
    ax.set_title(r"Reachable drain I–V: $I_{\mathrm{D}}$ vs $V_{\mathrm{DS}}$ (one series per $V_{\mathrm{GS}}$ bin)")
