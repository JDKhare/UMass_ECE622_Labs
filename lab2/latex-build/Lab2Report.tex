...
Schedule from simulation trace has the following schedule of vikings. First 5 and 10 start together and 5 returns with the torch. Then 20 and 25 start together and 20 returns with the torch. Finally, 5 and 10 start together and 5 returns with the torch. The total completion time is 60 minutes. Following is the table of the schedule:
...
\bottomrule
\end{tabular}
\caption{A.2 crossing schedule (different ordering of Vikings also possible; total completion \SI{75}{min}).}
...
We determined satisfiability by incrementally changing the time bound. The shortest completion time observed for this model is \SI{70}{min}, and a simulation trace (with a satisfying schedule) exists for the query above.
...
\bottomrule
\end{tabular}
\caption{A.3 query reaching completion at \SI{70}{min}. (Bridge selection shown via \texttt{B} in the table of schedule.)}
\label{tab:a3_schedule}
\end{table}

\labfigure[0.92]{PARTA3CorrectedPropertySatisfied.png}{A.3: Verifier screenshot showing the bounded reachability query is satisfied for \texttt{time <= 70}.}{fig:a3_query_satisfied}
...
\labfigure[0.55]{PartB-b5-at60_clause.png}{B.5: corresponding declaration line for the unsafe condition at \texttt{pedCrossTime=60}.}{fig:b5_at60_clause}
...
\appendix
\section*{Appendix: Submission Files, AI Tool Usage and Setup Details}
