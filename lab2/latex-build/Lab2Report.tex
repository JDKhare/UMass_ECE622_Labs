...
\begin{itemize}
\item In the Torch template, an additional path was added so up to three Vikings can take the torch for the new-bridge crossing case.
\item An urgent location in the Torch template forces this three-Viking path to execute without time elapse at that decision point.
\item In the Viking (Soldier) template, a second release path was added with a different delay on the release synchronization. This encodes the per-path timing difference while still merging at the same destination location.
\end{itemize}

The query evidence for the time bound is:
\begin{lstlisting}[style=uppaal]
...
Case & Bound & Result \\
\midrule
1 & \texttt{time <= 70} & Not satisfied (no execution reaches all five safe within 70 seconds). \\
2 & \texttt{time <= 75} & Satisfied (feasible witness trace exists). \\
\bottomrule
\end{tabular}
\caption{A.3 bounded-reachability check: 75 seconds is feasible, 70 seconds is not.}
\label{tab:a3_bound_check}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{c >{\raggedright\arraybackslash}p{0.31\textwidth} c c c}
\toprule
\# & Trip (Vikings by index; times in seconds) & Start time (\si{s}) & Trip origin & Bridge used \\
\midrule
1 & $\Vik{1}+\Vik{2}$ cross to safe side (5+10) & 0 & Unsafe side & Original \\
2 & $\Vik{1}$ returns with torch (5) & 10 & Safe side & Original \\
3 & $\Vik{1}+\Vik{5}$ cross to safe side (5+10) & 15 & Unsafe side & Original \\
4 & $\Vik{1}$ returns with torch (5) & 25 & Safe side & Original \\
5 & $\Vik{3}+\Vik{4}$ cross to safe side (20+25) & 30 & Unsafe side & Original \\
6 & $\Vik{2}$ returns with torch (10) & 55 & Safe side & Original \\
7 & $\Vik{1}+\Vik{2}$ cross to safe side (5+10) & 65 & Unsafe side & Original \\
\bottomrule
\end{tabular}
\caption{A.3 witness schedule reaching completion at 75 seconds. (Simulation screenshots omitted by request.)}
